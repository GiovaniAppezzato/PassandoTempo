<x-layout title="{{ $usuario != null ? $usuario->name : 'Usuário inexistente' }}"
          sidebarActive="{{ $edit ? '#usuario' : '' }}">

    @if($usuario)
        <div class="py-6 lg:py-8 relative">
            @if ($usuario->perfil->imagem_perfil != null)
                <img class="object-cover rounded-full select-none w-28 h-28 mx-auto shadow-lg drop-shadow-md lg:w-32 lg:h-32 xl:w-36 xl:h-36" src="{{ asset('storage/perfil/' . $usuario->perfil->imagem_perfil) }}" alt="imagem_perfil">
            @else
                <div class="flex justify-center items-center bg-gray-100 rounded-full w-28 h-28 mx-auto shadow-md lg:w-32 lg:h-32 xl:w-36 xl:h-36">
